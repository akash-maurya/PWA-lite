      ...
      <div className={style.container}>
        <div className={style.layer}>
          ...
          <div className={style.logo}>
            <Image src={logo} alt="licious logo"></Image>
          </div >
        
         <button onClick = {handleclick} className ={style.toggleButton}><FontAwesomeIcon className = {style.cross} icon = {faTimes} ></FontAwesomeIcon></button>
       
         </div>
          <div className={style.login_box}>
            <form className={style.form}>
              <h2 className={style.animate}>Sign In Here</h2>
              ...
              {showOTPbox && (
                <input type="password" onChange = {handleChangeOTP} placeholder="Enter your OTP" />
              )}

              {showInvalid && <button disabled id ={style.invalid}>Invlid number </button>}
              ...
              {userConfirmed && <button disabled id= {style.specialButton}>LoggedIn successfully</button>}

              {!showOTPbox && <button  type="submit" onClick={handleSubmit}>
                Proceed Via OTP
              </button>}

               {showOTPbox && <button onClick = {ConfirmOTP}>Confirm OTP</button>}
